...
This app expects PocketBase at `POCKETBASE_URL` and uses one auth collection plus one base collection.
...
- Public pages read `events` anonymously, so the list/view rules must stay open.
